...
def _pack_stream0_s16(vertex_count, uvs=None):
    ...
    for i in range(vertex_count):
        ...
        # SANITIZED: Prevents DX12 Access Violation
        out += struct.pack('<IIff', 0x00000000, 0x00000000, u, v)
    return bytes(out)
...
def _pack_stream0_s20_white(vertex_count, uvs=None):
    ...
    for i in range(vertex_count):
        u, v = uvs[i] if uvs else (0.0, 0.0)
        # SANITIZED: Prevents DX12 Access Violation
        out += struct.pack('<IIffI', 0x00000000, 0x00000000, u, v, 0)
    return bytes(out)
...
def _pack_stream0_dynamic(vertex_count, stride, uvs=None):
    ...
    for i in range(vertex_count):
        u, v = uvs[i] if uvs else (0.0, 0.0)
        # SANITIZED dynamic prefix
        record = bytearray(struct.pack('<IIff', 0x00000000, 0x00000000, u, v))
        if stride >= 24:
